  ...
  return (
    ...
    >
      ...
      <div style={{ margin: "0 16px" }}>
        ...
        >
          AI checks for signature and stamp — for your own review before sending
          to factoring, not a guarantee of approval.
        </div>
      </div>
...
function RouteMap({ from, to, extraPickups, extraDeliveries, miles, theme }) {
  // Мульти-стоп: класичний Google Maps URL підтримує проміжні точки
  // через "+to:" між адресами в daddr — без API-ключа, той самий підхід
  // що ми вже використовуємо для звичайного двоточкового маршруту.
  const middleStops = [
    ...(extraPickups || []).map((p) => p.city),
    ...(extraDeliveries || []).map((d) => d.city),
  ].filter(Boolean);
  const daddrParts = [...middleStops, to].map(encodeURIComponent);
  const daddr = daddrParts.join("+to:");

  const src = `https://www.google.com/maps?saddr=${encodeURIComponent(
    from,
  )}&daddr=${daddr}&output=embed&hl=en`;
  const openUrl = `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(
    from,
  )}&destination=${encodeURIComponent(to)}&waypoints=${middleStops.map(encodeURIComponent).join("|")}`;
...
function StopsList({ load }) {
  const allStops = [
    {
      type: "pickup",
      city: load.from,
      address: load.fromAddress,
      contactName: load.fromShipperName,
      contactPhone: load.fromShipperContact,
    },
    ...(load.extraPickups || []).map((p) => ({
      type: "pickup",
      city: p.city,
      address: p.address,
    ...
    })),
    {
      ...
      contactName: load.toReceiverName,
      contactPhone: load.toReceiverContact,
    },
  ];

  // Для звичайного лоуда (2 стопи) окремий список не потрібен —
  // RouteMap і Row-поля вже все показують.
      ...
      {allStops.map((stop, i) => (
        <div
          ...
          style={{
            ...
            padding: "8px 0",
            borderBottom:
              i < allStops.length - 1 ? "1px solid var(--border)" : "none",
          }}
        >
          <div
            style={{
              ...
              flexShrink: 0,
              marginTop: 5,
              background: stop.type === "pickup" ? "#4ade80" : "var(--accent)",
            }}
          />
          ...
          <div
            style={{
              ...
              fontSize: 9,
              letterSpacing: "0.08em",
              color: stop.type === "pickup" ? "#4ade80" : "var(--accent)",
              flexShrink: 0,
              marginTop: 3,
            }}
          >
